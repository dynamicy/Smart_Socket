/*1351566960,178142531*/

if (self.CavalryLogger) { CavalryLogger.start_js(["Mlcc6"]); }
...
__d("DOMEvent",["copyProperties","Arbiter"],function(a,b,c,d,e,f){
...
return new i(this)._qualify();},


_qualify:function(){
if(!this.getDomain()){
var j=i();
this.setProtocol(j.getProtocol()).
setDomain(j.getDomain()).
setPort(j.getPort());}
...
var k=this._qualify().getDomain().split('.');
if(k.length<=2){
k.unshift(j);}else 

k[0]=j;

return this.setDomain(k.join('.'));},
...
this._enabledQueryCache=o.enabledQueryCache||true;
this._queryExactMatch=o.queryExactMatch||false;
...
var s=this.buildUids(o,[],q),
t=this.respond(o,s);

this.value=o;
this.inform('query',{value:o,results:t});

var u=i.parse(o).flatValue;
if(p||
!u||
!this.queryEndpoint||
this.getQueryCache().hasOwnProperty(u)||
!this.shouldFetchMoreResults(t))
return false;
...
if(!o.titleToIndex)
o.titleToIndex=this.getTextToIndexFromFields(o,this._titleFields);
...
p.push('<span class="emote_text" aria-hidden="true">');
p.push(t);
p.push('</span>');
var w,
...
o.ajaxReqSend=o.send;
o.send=function(q){
this.ajaxReqSend(i(q,n));};
...
__d("MercuryServerDispatcher",["AsyncRequest","FBAjaxRequest","Env","JSLogger","URI","areObjectsEqual","copyProperties","debounceAcrossTransitions"],function(a,b,c,d,e,f){
...
trySend:function(t,u,v){
o[t].trySend(u,v);}};



function s(t,u){
var v=u.mode||r.IMMEDIATE;
...
default:throw new Error('Invalid MercuryServerDispatcher mode '+v);
}
...
if(v==r.BATCH_SUCCESSIVE||
v==r.BATCH_SUCCESSIVE_UNIQUE){
if(!this._inFlight){
this._send(t);}else 

this._batchData(t);}else

if(v==r.BATCH_CONDITIONAL){
...
if(this._inFlight&&
(this._combineDataIf(this._pendingRequestData,t)||
this._combineDataIf(this._batchedData,t))){
this._batchData(t);}else 

this._send(t);}else

if(v==r.BATCH_DEFERRED_MULTI){
this._batchData(t);
this._deferredSend();}else
if(v==r.BATCH_SUCCESSIVE_PIGGYBACK_ON_ERROR){
...
_send:function(t){
this._inFlight++;
this._pendingRequestData=m({},t);

p.log('send',{endpoint:this._endpoint,
data:t,
...
u=new h('POST',this._endpoint,t);
u.onError=function(v){
v.getPayload=function(){
return v.errorText;};

v.getRequest=function(){
var w=v;
v.getData=function(){
return t;};
...
this._handleResponse(v);}.
bind(this);
u.send();}else{

u=new g(this._endpoint).
setData(t).
...
_batchData:function(t){
if(this._mode==r.BATCH_SUCCESSIVE_UNIQUE&&
typeof this._pendingRequestData!='undefined'&&
l(t,this._pendingRequestData)){



return;}else
if(typeof this._batchedData=='undefined'){
this._batchedData=t;}else 

this._batchedData=this._combineData(this._batchedData,t);},
...
this._inFlight--;
p.log('response',{endpoint:this._endpoint,
inflight_count:this._inFlight});
...
this._handler&&this._handler(u);


if(this._mode==r.BATCH_SUCCESSIVE||
this._mode==r.BATCH_SUCCESSIVE_UNIQUE||
this._mode==r.BATCH_SUCCESSIVE_PIGGYBACK_ON_ERROR||
this._mode==r.BATCH_CONDITIONAL)
this._batchSend();
...
p.error('error',{endpoint:this._endpoint,
inflight_count:this._inFlight-1});
this._inFlight--;

var t=this._mode;
if(t==r.BATCH_SUCCESSIVE||
t==r.BATCH_SUCCESSIVE_UNIQUE||
t==r.BATCH_CONDITIONAL){
this._batchSend();}else
if(t==r.BATCH_SUCCESSIVE_PIGGYBACK_ON_ERROR)
...
this._serverDialogCancelHandler&&
this._serverDialogCancelHandler(t);
this._postErrorHandler();}});
