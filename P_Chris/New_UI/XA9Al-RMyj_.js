/*1351556638,173213213*/

if (self.CavalryLogger) { CavalryLogger.start_js(["j2Y1E"]); }

/** js/polyfill/Array.js */


/** js/polyfill/Array.prototype.js */


/** js/polyfill/Date.js */


/** js/polyfill/__DEV__.js */
...
/** js/polyfill/Function.prototype.js */


/** js/polyfill/Function.prototype-shield.js */
...
/** js/polyfill/Object.js */


/** js/polyfill/Object-shield.js */
...
/** js/polyfill/Object.prototype.js */


/** js/polyfill/String.prototype.js */


/** js/polyfill/String.prototype.split.js */


/** js/polyfill/TypeChecker.js */
...
g='DOMElement';
h=f.nodeName.toUpperCase();}else{


g=a.call(f).slice(8,-1);
switch(g){
case 'Array':
if(f.length)
h=c(f[0]);
...
if(/null|undefined/.test(g)&&/\?$/.test(f))
return true;
...
f=f.replace(/>*$|\?|$/,'<');
g=g.replace(/>*$|$/,'<');
var h=Math.min(f.length,g.length);

return f.substring(0,h)===g.substring(0,h);}
...
var h=f[g][1],
i=c(f[g][0]),
j=f[g][2]||'return value';

if(!d(h,i)){
var k=new TypeError('Type Mismatch for '+j+': expected '+
h+', actual '+i);
if(b){
...
e.setHandler=function(f){
b=f;};

return e;})
();

/** js/polyfill/console.js */


/** js/polyfill/require.js */
...
v=['global','require','requireDynamic','requireLazy','module',
'exports'].concat(v);
t(u,v,w,x);};
...
document.documentElement.className.replace('no_js','');

/** js/chat/sidebar/SidebarPrelude.js */
...
/** js/modules/core/copyProperties.js */
...
if(q.hasOwnProperty&&q.hasOwnProperty('toString')&&
(typeof q.toString!='undefined')&&(h.toString!==q.toString))
...
var z=
{line:y.lineNumber||y.line,
message:y.message,
name:y.name,
script:y.fileName||y.sourceURL||y.script,
stack:m(y.stackTrace||y.stack)};
...
if(y.framesToPop){
var aa=z.stack.split('\n');
aa.splice(0,y.framesToPop);
z.stack=aa.join('\n');
if(/(\w{3,5}:\/\/[^:]+):(\d+)/.test(aa[0])){
z.script=RegExp.$1;
z.line=parseInt(RegExp.$2,10);}}
...
y){
var z=n(y).stack;
return z&&z.replace(/[\s\S]*__getTrace__.*\n/,'');}}
...
emptyFunction(y._originalError);
...
if(y)
fa.callee=y.toString().substring(0,100);

if(aa)
fa.args=String(aa).substring(0,100);
...
/** js/modules/core/hasArrayNature.js */
...
/** js/modules/core/createArrayFrom.js */
...
window.async_callback=function(m,n){
return m;};
...
q._events[t].push(n);
q._events[t]._stateful=false;}else
if(o==k.BEHAVIOR_STATE){
u=0;
q._events[t]=[n];
q._events[t]._stateful=true;}else
if(t in q._events)
...
a.ArbiterMonitor&&
a.ArbiterMonitor.record('event',t,n,q);
var v;
if(q._index[t]){
var w=i(q._index[t]);
...
if(n._events[m].length)
return n._events[m][0];
...
if(n===null||!this._listen[m])
return;


for(var o in this._listen[m]){
this._listen[m][o]--;
if(this._listen[m][o]<=0)
delete this._listen[m][o];
...
/** js/deprecated/ArbiterMixin-legacy.js */
...
/** js/modules/core/emptyFunction.js */
...
thatReturnsArgument:function(j){return j;}});
...
if(l[t])
throw new Error
('CSS component '+t+' has been requested and should not be '+
...
v.parentNode.removeChild(v);}else{
...
ca.async=true;
var da=p[z];
if(da&&da.crossOrigin&&i.crossorigin_attribute)
...
aa.appendChild(ca);}else
if(x=='css')
h.loadStyleSheet(z,y,aa,ba,function(){
...
window.CavalryLogger.getInstance().measureResources(ga,aa);}}



if(y)
y=g.registerCallback(y,fa);




var ia=document.documentMode||
+(/MSIE.(\d+)/.exec(navigator.userAgent)||[])[1],
ja=w.getHardpoint(),
ka=ia?ja:document.createDocumentFragment();

for(ba=0;ba<ea.length;++ba)
t
(ea[ba].type,
ea[ba].src,
ea[ba].name,
...
var h=
(a.Event.__getHandler&&
a.Event.__getHandler(g,'submit'));
...
h=g.onsubmit?
g.onsubmit(event):
a.Event.__fire&&a.Event.__fire(g,'submit',event);
...
var y=a.OnloadHooks;
if(window.loaded&&y){
y.runHook(x,'onlateloadhooks');}else 
...
y?
q('onbeforeleavehooks',x):
q('onbeforeunloadhooks',x);}
...
x.addEventListener("DOMContentLoaded",s,true);}else{


var ba='javascript:void(0)';
if(y.location.protocol=='https:')
...
if(false!==n()&&l.href)
window.location.href=l.href;}else
...
return j.match(new RegExp(h.punct_char_class+
'['+
')"'+
"'"+
...
return t.slice(1).toLowerCase();});}else 
...
/** js/modules/ServerJS.js */
__d("ServerJS",["function-extensions","ErrorUtils","copyProperties","ge"],function(a,b,c,d,e,f){
...
k(n.define||[],this._handleDefine,this);
k(n.markup||[],this._handleMarkup,this);
k(n.elements||[],this._handleElement,this);
k(n.instances||[],this._handleInstance,this);
k(n.require||[],this._handleRequire,this);}),
...
handlePartial:function(n){
(n.instances||[]).forEach(l.curry(this._moduleMap,3));
...
this._replaceTransportMarkers(p,q);}else
...
n[o]=i(p.__e);}else
if(p.__rel){
n[o]=this._relativeTo;}else 

for(var r in p)
this._replaceTransportMarkers(p,r);}});
...
function y(ba){
if(!ba.firstChild){
emptyFunction('Pagelet markup container is empty.');
...
emptyFunction('Child of pagelet markup container is not a comment.');
return null;}
...
lid:0,
isAjax:false,
domContentCallback:m.__domContentCallback,
onloadCallback:m.__onloadCallback,
...
if(this.automatic){
this._relevant_instance=aa._current_instance;}else 
...
g.inform('BigPipe/init',{lid:this.lid,arbiter:this.arbiter},
v);
...
this.displayCallback(this._displayPagelet.bind(this,ba));}else 
...
if(!fa)
ea.setAttribute('data-referrer',ca);


if(ba.cache_hit&&i.pc_debug)
ea.style.border='1px red solid';}}
...
return (ba.append==='bigpipe_root')?
this.rootNodeID:ba.append;},
...
ba.provides&&
this.arbiter.inform(ba.provides,true,u);}.
bind(this),

da=function(){
this._isRelevantPagelet(ba)&&
s(ba.onafterload);}.
bind(this);
...
var da=ba.display_dependency||[],
ea=[];
for(var fa=0;fa<da.length;fa++)
ea.push(da[fa]+'_displayed');

this.arbiter.registerCallback
(this._displayPageletHandler.bind(this,ba),ea);}.
bind(this),false,ba.id);},
...
this._phaseDoneCallbacks[ca]=
this.arbiter.registerCallback(this._onPhaseDone.bind(this),
['phase_complete_'+ca]);


this.arbiter.registerCallback(this._phaseDoneCallbacks[ca],
[ba.id+'_displayed']);

var da=a.PageletCache;
if(da)
if(ba.cache_hit){
ba=da.loadFromCache(ba,this.arbiter);}else 

da.write(ba,this.arbiter);
...
var ca=ba.phase,
da=this._getAppendTargetID(ba)||ba.id;
aa.pageletIDs[da]=da;

if(ba.the_end)
this._lastPhase=ba.phase;
...
this._secondToLastPhase=ba.phase;
...
this.arbiter.registerCallback
(this._downloadDisplayDependenciesAndDisplayPagelet.bind(this,ba),
[fa]);

var ga=[ba.id+'_displayed'];
if(!this.jsNonBlock)
ga.push(this.domContentEvt);


this.arbiter.registerCallback(this.onloadCallback,['pagelet_onload']);

this.arbiter.registerCallback
(this._downloadJsForPagelet.bind(this,ba),ga);


ba.is_last&&this.arbiter.inform('phase_complete_'+ca,
true,u);},
...
this._informEventExternal('tti_bigpipe',{phase:this._ttiPhase});


var ba=this._currentPhase+1,
...
this.arbiter.inform('pagelet_displayed_all',true,u);
...
return this==aa._current_instance||
(this.automatic&&
...
var v=g.registerCallback
(o.curry(t,u),
[g.FUNCTION_EXTENSION+'/'+u,g.BOOTLOAD]);
...
g.inform(g.FUNCTION_EXTENSION+'/'+t,
true,
g.BEHAVIOR_STATE);}else 
...
w=p
(t,
v.prototype.__level+1);
w.parent=v;
if(!!t.__class_name)
g.inform
(g.FUNCTION_EXTENSION+'/'+t.__class_name,
true,
g.BEHAVIOR_STATE);}
...
for(var w in v)
if(w!='__level'&&w!='construct'&&u[w]===undefined)
u[w]=t.prototype[w]=v[w];}
...
for(var w in u)
if(w!='parent')
u[w]=t.prototype[w]=s(u[w],x);}




function s(t,u,v){
if(typeof t!='function'||t.__prototyped)
return t;
...
/** js/listeners/modules/PlaceholderListener.js */
...
j.addEventListener('blur',k,true);}else{
...
while(k--){
var l=j[k].split('=',2);
i[decodeURIComponent(l[0])]=l.length===2?
...
try{return a.XMLHttpRequest?
new a.XMLHttpRequest():new ActiveXObject("MSXML2.XMLHTTP.3.0");}catch(
i){}},
...
__d("BanzaiAdapter",["Arbiter","Miny","QueryString","Run","XHR","BanzaiConfig"],function(a,b,c,d,e,f){
...
var w=l.status;
q.cleanup();
q.inform(w==200?p.OK:p.ERROR);}};




setTimeout(q.cleanup,p.SEND_TIMEOUT,false);


var t=k.getAsyncParams(s);
...
if(p.isEnabled('miny_compression')){
var u=Date.now(),
v=h.encode(t.q);
...
/** js/logging/banzai/Banzai.js */
__d("Banzai",["BanzaiAdapter","pageID"],function(a,b,c,d,e,f){
...
var g=b('BanzaiAdapter'),h=g.adapter,
i=b('pageID'),
...
if(n.length<=0&&m.posts.length<=0){
...
o();
s.setItem(j,JSON.stringify(n));},

restore:function t(){
var u=s.getItem(j);
if(u){
s.removeItem(j);
n=n.concat(JSON.parse(u));}}};}else 
...
g.VITAL={store:true,delay:h.config.MIN_WAIT||1000};
g.BASIC={store:false,delay:h.config.MAX_WAIT||60000};

g.FBTRACE=h.config.fbtrace,

g.isEnabled=function(t){





return h.config.gks&&h.config.gks[t];};



g.post=function(t,u,v){
v=v||g.BASIC;
...
x[this._ns]:{},
z=y?
y[this._ua_id]:undefined;

if(h.isEnabled('useraction')&&
(z!==undefined?
z:w))
h.post(u,v,o);},
...
return (this._primary._action_type||
this._fallback._action_type||
this._default_action_type);},


_get_ua_id:function(){
return (this._primary._ua_id||
this._fallback._ua_id||
this._default_ua_id);},
...
t=j.byAttribute(t.parentNode,u));
return false;}
...
p.addEventListener('focus',s.curry('focus'),true);}else 

p.attachEvent('onfocusin',s.curry('focus'));});

/** js/modules/QuicklingPrelude.js */
...
document.cookie="rdir="+window._script_path+"; path=/; domain="+
window.location.hostname.replace(/^.*(\.facebook\..*)$/i,'$1');
...
k=
i.keyCode==13&&
!i.altKey&&!i.ctrlKey&&!i.metaKey&&!i.shiftKey&&
h.hasClass(j,'enter_submit');
if(k){
g.loadModules
...
var p=j.form,
q=
l.scry(p,'.enter_submit_target')[0]||
l.scry(p,'[type="submit"]')[0];
if(q){
var r=
o.getAttribute(p,'ajaxify')||
o.getAttribute(p,'action');
if(r)
n(p,r);
...
window.ScrollAwareDOM.monitor(q,r);}else 

r();

if(p!==false)
(q.add_comment_text_text||q.add_comment_text).focus();
...
if(window.ScrollAwareDOM){
window.ScrollAwareDOM.monitor(o,p);}else 
