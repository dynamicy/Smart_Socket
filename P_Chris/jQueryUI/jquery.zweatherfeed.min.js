(function (i) {
    i.fn.weatherfeed = function (o, h, t) {
        var h = i.extend({
            ...
            p = "odd";
        return this.each(function (m, q) {
            var k = i(q);
            k.hasClass("weatherFeed") || k.addClass("weatherFeed");
            if (!i.isArray(o)) return !1;
            var l = o.length;
            10 < l && (l = 10);
            for (var j = "", m = 0; m < l; m++)"" != j && (j += ","), j += "'" + o[m] + "'";
            now = new Date;
            l = "http://query.yahooapis.com/v1/public/yql?q=" + encodeURIComponent("select * from weather.forecast where " + (h.woeid ? "woeid" : "location") + " in (" + j + ") and u='" + h.unit + "'") + "&rnd=" + now.getFullYear() + now.getMonth() + now.getDay() + now.getHours() + "&format=json&callback=?";
            i.ajax({
                type: "GET",
                url: l,
                dataType: "json",
                success: function (f) {
                    if (f.query) {
                        if (0 < f.query.results.channel.length) for (var c = f.query.results.channel.length, e = 0; e < c; e++) u(q, f.query.results.channel[e], h);
                        else u(q, f.query.results.channel, h);
                        i.isFunction(t) && t.call(this, k)
                    } else h.showerror && k.html("<p>Weather information unavailable</p>")
                },
                error: function () {
                    h.showerror && k.html("<p>Weather request failed</p>")
                }
            });
